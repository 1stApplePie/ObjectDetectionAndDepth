...
CFG = "/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/yolov3-tiny.cfg"
TRT = '/home/nvidia/xycar_ws/src/yolov3_trt_ros/src/model_epoch2000.trt'
NUM_CLASS = 8
INPUT_IMG = '/home/nvidia/xycar_ws/src/yolov3-tiny/datasets/data/tstl_data/JPEGImages/cal_000770.png'

with open('/home/nvidia/xycar_ws/src/ObjectDetectionSystem/config/config.yaml', encoding = 'UTF-8') as f:
...
dist_mtx = np.array([-0.299279, 0.067634, -0.001693, -0.002189, 0.0])

cal_mtx, cal_roi = cv2.getOptimalNewCameraMatrix(calibrate_mtx, dist_mtx, (640, 480), 1, (640, 480))

x, y, w, h = cal_roi

map1, map2 = cv2.initUndistortRectifyMap(calibrate_mtx, dist_mtx, None, cal_mtx, (640, 480), cv2.CV_32FC1)

class yolov3_trt(object):
    def __init__(self, mdebug = mdebug):
        self.cfg_file_path = CFG
        self.num_class = NUM_CLASS
        width, height, masks, anchors = parse_cfg_wh(self.cfg_file_path)
        self.engine_file_path = TRT
        self.show_img = mdebug
        ...
        postprocessor_args = {"yolo_masks": masks,                    # A list of 3 three-dimensional tuples for the YOLO masks
                              ...
                              "nms_threshold": 0.15,                                               # Threshold for non-max suppression algorithm, float value between 0 and 1
    ...
    def _write_message(self, detection_results, boxes, scores, classes):
        origin_img_width = 640
        origin_img_height = 480
        yolo_img_length = 416
        ...
        for box, score, category in zip(boxes, scores, classes):
            ...
            detection_msg.xmin = int(minx * origin_img_width / yolo_img_length)
            detection_msg.xmax = min(origin_img_width, int((minx + width) * origin_img_width / yolo_img_length))
            detection_msg.ymin = int(miny * origin_img_height / yolo_img_length)
            detection_msg.ymax = min(480, int((miny + height) * origin_img_height / yolo_img_length))
...
def calibrate_image(src):
    ...
    dst = dst[y : y + h, x : x + w]
    return cv2.resize(dst, (640, 480))

def img_callback(msg):
    global xycar_image
    data = np.frombuffer(msg.data, dtype=np.uint8)
    src = np.reshape(data, (msg.height, msg.width, 3))
    src_image = cv2.cvtColor(src, cv2.COLOR_RGB2BGR)
    xycar_image = calibrate_image(src_image)
